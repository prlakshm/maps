...
export function REPLInput(props: REPLInputProps) {
  ...
  const handleLoad: REPLFunction = async (args: string[]): Promise<string> => {
    ...
    try {
      const response = await fetch(
        `https://cs32customserver.com/maps/loadcsv?filepath=${filepath}`
      );
      if (response.ok) {
  ...
  const handleView: REPLFunction = async (args: string[]): Promise<string> => {
    ...
    try {
      const response = await fetch("https://cs32customserver.com/maps/viewcsv");
      if (response.ok) {
        const data = await response.json();
        const resultMessage =
  ...
  const handleSearch: REPLFunction = async (
    ...
    try {
      const response = await fetch(
        `https://cs32customserver.com/maps/searchcsv?headers=${hasHeaders}&value=${value}&colid=${columnId}`
      );
      if (response.ok) {
        const data = await response.json();
        const resultMessage =
  ...
  const handleBroadband: REPLFunction = async (
    ...
    try {
      const response = await fetch(
        `https://cs32customserver.com/maps/broadband?state=${state}&county=${county}`
      );
      if (response.ok) {
        const data = await response.json();
        const resultMessage =
  ...
  /**
   * Function handling search areas for highlighting map
   * @param {string[]} args - keyword to search areas
   */
  const handleAreas: REPLFunction = async (args: string[]): Promise<string> => {
    if (args.length !== 1) {
